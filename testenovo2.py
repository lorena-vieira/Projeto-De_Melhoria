        ...
        if self.inadequacoes_contraventamento.get():
            for paragraph in doc.paragraphs:
                if marcador in paragraph.text:
                    # Substituir o marcador pelo novo texto

                    marcador = "[TextoInadequacoes]"
                    novo_texto = ("Os contraventamentos possuem configurações e características não recomendadas, como indicado a seguir. "
                    ...
                    paragraph.text = paragraph.text.replace(marcador, novo_texto)



        # Salvar o documento modificado
        output_path = f"Relatorio_novo.docx"
        doc.save(output_path)

        # Exibir mensagem de sucesso
        messagebox.showinfo("Relatório Gerado", f"Relatório para a Ponte criado com sucesso! Salvo em {output_path}")
